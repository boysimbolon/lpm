                            ...
                            <tbody>
                                <tr v-for="(tipe_audit, index) in tb_tipe_audit" :key="tipe_audit.id" class="border-y">
                                    ...
                                    <td class="p-4 flex flex-col lg:flex-row items-center gap-4 justify-center">
                                        <router-link :to="{ name: 'StandarListAsAdmin' }" @click="view(tipe_audit)" class="py-2 px-4 text-white rounded bg-blue-500 hover:bg-blue-700 shadow-md">View</router-link>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
...
document.title = `Tipe Audit - Standar List | ${authStore.name}`;
...
onMounted(async () => {
    ...
    const response = await axios.get(`${authStore.url}/tipeAudits`, authStore.header);
