                            ...
                            <tbody class="divide-y">
                                ...
                                <tr v-for="(baris, index) in rows" :key="baris" class="odd:bg-white even:bg-gray-100 hover:bg-gray-100">
                                    ...
                                    <td class="px-6 py-4 w-[70%] text-gray-800 text-start">
                                        <textarea type="text" v-model="baris.Pernyataan" v-if="baris.pernyataan === null" class="rounded border border-black border-solid w-full"></textarea>
...
const rows = ref([]);
document.title = `Create Pernyataan Standar | ${authStore.name}`;
...
const updateRows = async () => {
    ...
    if (standar !== null && tipeAudit !== null) {
        try {
            const response = await axios.get(`${authStore.url}/per-standars/${tipeAudit}/${standar}`,
             authStore.header);
...
const Edit = (standar, tipeAuditParam, standarParam) => {
    ...
    router.push({ name: 'EditPernyataanStandarAsAdmin' });
}
</script>
