        ...
        <div class="bg-white w-3/4 rounded-xl flex flex-col divide-y-2 shadow-md">
            ...
            <form @submit.prevent="createStandar" class="py-4 flex flex-col gap-4 divide-y-2">
                <div class="flex flex-col gap-4">
                    ...
                    <!-- Tipe Audit -->
                    <div class="flex flex-col gap-1 px-4">
                        <label class="font-medium" for="role">Tipe Audit</label>
                        <div class="flex items-center divide-x-[1px] divide-black gap-2 pl-2 border border-black rounded bg-neutral-300">
                            ...
                            <select v-model="form.idtipe" name="role" id="role" class="p-2 w-full focus:outline-none capitalize">
                                <option v-for="tipe in tb_tipeAudit" :key="tipe.id" :value="tipe.id">{{ tipe.tipe_audit }} - {{tipe.periode}} - {{tipe.semester}}</option>
                            </select>
                        </div>
                    </div>
                </div>
...
onMounted(async () => {
    ...
    if(authStore.role == 'auditor'){
        ...
        tb_tipeAudit.value = response.data;
    }
});
