...
<!--            <p>Mauris fermentum tortor a diam eleifend scelerisque.</p>-->
        ...
        <div class="bg-white rounded-xl gap-4 shadow-md divide-y-[1.5px] hidden" id="tabel">
            ...
            <div class="p-4 flex items-center justify-between">
                ...
                <div class="flex items-center justify-between">
                    <select class="bg-sky-500 py-2 px-4 rounded-lg text-white transition-all duration-300 ease-in-out hover:bg-sky-700 hover:shadow-lg truncate w-3/4" v-model="form.standar" @change="detail1()">
                        <option disabled class="bg-[#e0dded] py-2">Choose an option</option>
            ...
            <div class="overflow-x-auto">
                <div class="p-1.5 min-w-full inline-block align-middle">
                        ...
                        <table class="min-w-full relative">
                            <thead>
                                <tr class="sticky top-0 bg-gray-200">
                                    <th scope="col" class="sticky top-0 px-6 py-3 text-start text-gray-500">Pernyataan</th>
                                    <th scope="col" class="sticky top-0 px-6 py-3 text-start text-gray-500">Dokumen Pendukung</th>
                                    <th scope="col" class="sticky top-0 px-6 py-3 text-center text-gray-500">Nilai Diri</th>
                                    <th scope="col" class="sticky top-0 px-6 py-3 text-center text-gray-500">Nilai Maksimum</th>
                                    <th scope="col" class="sticky top-0 px-6 py-3 text-center text-gray-500">Status</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y">
                                <tr class="odd:bg-white even:bg-gray-100 hover:bg-gray-100 border-y" v-for="detail in details" :key="detail">
                                    ...
                                    <td class="px-6 py-4 text-center text-gray-800">{{ detail.nilai_diri }}</td>
                                    <td class="px-6 py-4 text-center text-gray-800">{{ detail.per_standars.poin }}</td>
                    ...
                    <!-- Unlock Button -->
                    <button class="py-2 px-4 bg-blue-500 text-white w-fit rounded my-3 hover:bg-blue-700 shadow-md" @click="Open(details)" v-if="form.standar != 'ALL' && status">Buka Kunci</button>
                </div>
            </div>
        </div>
...
const req = onMounted(async () => {
    try {
        ...
        const [resAudit, resTipe, resStandar] = await Promise.all([
            axios.get(`${authStore.url}/Myauditi/${authStore.uid}`,authStore.header),
...
const detail1 = () => {
    details.value = [];
    status.value = true; // Default value

    datas.value.forEach(data => {
        const isAuditorMatched = data.users.name == auditor.value;
        const isAuditTypeMatched = data.id_tipe_audits == form.value.tipe;
        const isStandardMatched = data.per_standars.id_standars == form.value.standar;
        const isAllStandards = form.value.standar == "ALL";

        if (isAuditorMatched && isAuditTypeMatched && (isStandardMatched || isAllStandards)) {
