    ...
    try {
        if(form.value.tipe_audit != null && form.value.periode != null){
            if (tipe === "save") {
                ...
                const options = authStore.idParams.map(tipeAudit => `${tipeAudit.id}) ${tipeAudit.tipe_audit} - ${tipeAudit.fakultas.fakultas} - ${tipeAudit.fakultas.prodi} - ${tipeAudit.periode} - ${tipeAudit.semester} - ${tipeAudit.fakultas.fakultas} - ${tipeAudit.fakultas.prodi}`);
