        ...
        <div class="bg-white rounded-xl flex flex-col divide-y-2 shadow-md">
            ...
            <div class="p-4 flex items-center justify-between">
                ...
                <div class="flex gap-4 items-center">
                    <!-- Create Button -->
                    <router-link :to="{ name: 'CreateStandarAsAuditor', id: idParams.idParams.id }" class="flex items-center bg-sky-500 text-white py-2 px-4 rounded hover:bg-sky-700 shadow-md">
                            ...
                            <tbody>
                                <tr v-for="(standar,index) in tb_standar" :key="standar.id" class="border-y">
                                    ...
                                    <td class="p-4 flex flex-col lg:flex-row items-center justify-center gap-4">
                                        <!-- Edit Button -->
                                        <router-link :to="{ name: 'UpdateStandarAsAuditor', params: { id: idParams.idParams.id, id_tipe_audit: idParams.idParams.id_tipe_audit } }" @click="edit(standar, idParams.idParams.id, idParams.idParams.tipe_audit, idParams.idParams.periode, idParams.idParams.semester)" class="p-1 rounded border-2 border-blue-500 bg-blue-500">
    ...
    if (input !== null) {
        ...
        if (response.data.valid) {
            if (confirm("Tetap Yakin Untuk Menghapus Standar ini?")) {
                await axios.delete(`${authStore.url}/standars/${standar.id_standars}`, authStore.header);
                request();
            }
        } else {
...
// Back Method
const goBack = () => {
    router.push({name: 'StandarViewAsAuditor'});
}
</script>
